...
class ReviewData(BaseModel):
    ...
    JSON数据示例:
    {
      ...
      "审计证据标准": {
        ...
        "充分适当评判标准": "example_sufficiency_and_appropriateness_criteria"
      }
    }
    ...
    class AuditEvidenceStandard(BaseModel):
        ...
        充分适当评判标准: str = Field(..., description="充分适当评判标准")
...
class ReviewData(BaseModel):
    ...
    class AuditEvidenceStandard(BaseModel):
        ...
        审计结论: str = Field(..., description="审计结论")
        审计证据分类与要求: List[Dict[str, Dict[str, List[Dict[str, str]]]]] = Field(..., description="审计证据分类与要求")
        充分_适当评判标准: str = Field(..., description="充分适当评判标准")
...
class UserMessage(BaseModel):
    ...
    用户消息模型
    """
    status: str = Field(..., description="系统所处阶段")
    content: list[dict] = Field(..., description="对话历史消息")
...
class StandardsRequest(BaseModel):
    ...
    字段说明:
    - status: 当前系统状态（如：idle, analysis, processing, completed等）
    - type: 处理类型，可选值：
        - "ai": AI智能处理（语义检索 + 自动生成）
        - "user": 用户手动处理
    - content: 查询内容（审计结论或关键词）
    
    后端处理策略:
    - type="ai": 先尝试语义检索（相似度 >= 0.7），找不到则自动AI生成
    - type="user": 待定（根据实际业务需求处理）
    """
    status: str = Field(..., description="当前系统状态")
    type: str = Field(..., description="处理类型：ai 或 user", pattern="^(ai|user)$")
    content: str = Field(..., description="查询内容（审计结论或关键词）", min_length=1)
